...
import me.loveshare.data.exception.UDException;
...
public class RegularUtils {
     ...
     * 是否数字开头
     */
    public static final boolean isStartDigit(String string) {
        return Optional.ofNullable(string).map(s -> string.matches(RegExpDefinition.START_DIGIT)).orElseGet(() -> false);
    }
    ...
    /**
     * 用户名称  --4~25位、字母（大小写）、数字、@_.-
     */
    public static final boolean isNormalUsername(String username) {
        return Optional.ofNullable(username).map(n -> n.matches(RegExpDefinition.USERNAME)).orElseGet(() -> false);
    }
     ...
     */
    public static boolean isChineseIdentityCard(String identityCard) throws UDException {
        try {
            ...
            throw new UDException("身份证号码:" + identityCard + "有效性校验异常.", e);
        }
    }
     ...
     * @return true 数据验证通过
     */
    public static final boolean isChinesePassport(String code) {
        return Optional.ofNullable(code).map(c -> c.matches(RegExpDefinition.CHINESE_PASSPORT)).orElseGet(() -> false);
    }
     ...
     * @return true 数据验证通过
     */
    public static final boolean isChineseHongkongMacaoPass(String code) {
        return Optional.ofNullable(code).map(c -> c.matches(RegExpDefinition.CHINESE_HONGKONG_MACAO_PASS)).orElseGet(() -> false);
    }
     ...
     * @return true 数据验证通过
     */
    public static final boolean isChineseTaiwanPass(String code) {
        return Optional.ofNullable(code).map(c -> c.matches(RegExpDefinition.CHINESE_TAIWAN_PASS)).orElseGet(() -> false);
    }
     ...
     * @return true 数据验证通过
     */
    public static final boolean isPostCode(String code) {
        return Optional.ofNullable(code).map(c -> c.matches(RegExpDefinition.POST_CODE)).orElseGet(() -> false);
    }
     ...
     * @return true 数据验证通过
     */
    public static final boolean is13Barcode(String code) {
        return Optional.ofNullable(code).map(c -> c.matches(RegExpDefinition.BARCODE_13_BIT)).orElseGet(() -> false);
    }
    ...
    public static boolean isJsonString(String json) {
        return Optional.ofNullable(json).map(j -> j.matches(RegExpDefinition.OBJECT_JSON)).orElseGet(() -> false);
    }
